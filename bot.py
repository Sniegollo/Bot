...
@app_commands.choices(item=[
    app_commands.Choice(name=n, value=n) for n in CENNIK.keys()
])
...
async def on_message(message):
    ...
    if message.attachments:
        last = collection.find_one(
            {"user_id": message.author.id, "img": None},
            sort=[("id", -1)]
        )
        ...
        if last:
            collection.update_one(
                {"id": last["id"]},
                {"$set": {"img": message.attachments[0].url}}
            )

            await message.channel.send("📸 Screen przypisany!")
